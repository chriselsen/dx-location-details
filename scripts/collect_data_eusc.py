...
# Support both environment variable and CLI argument for profile
...
def get_dx_locations():
    """Fetch DX locations from EUSC regions"""
    eusc_regions = ['eusc-de-east-1']
    profile_arg = f"--profile {AWS_PROFILE}" if AWS_PROFILE else ""
    
    cmd = f"""
    for r in {' '.join(eusc_regions)}; do 
        aws directconnect describe-locations --region $r {profile_arg} \
        --query 'locations[]' \
...
def main():
    ...
    print("Fetching DX locations from EUSC...")
